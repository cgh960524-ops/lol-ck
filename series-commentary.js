...
const publicTimelineEvent=event=>({minute:maybeNumber(event.minute),team:publicTeamKey(event.side),kind:event.kind||"event",type:cleanText(event.type,30),subType:cleanText(event.subType,40),lane:cleanText(event.lane,30),tower:cleanText(event.tower,30),label:cleanText(event.label||[event.type,event.subType,event.lane,event.tower].filter(Boolean).join("/"),60)});

function timelineGoldAt(gold,minute){
  if(!gold.length)return null;
  let before=gold[0],after=gold.at(-1);for(const row of gold){if(n(row.minute)<=minute)before=row;if(n(row.minute)>=minute){after=row;break}}
  if(before===after||n(after.minute)<=n(before.minute))return n(before.team1Gold)-n(before.team2Gold);
  const ratio=Math.max(0,Math.min(1,(minute-n(before.minute))/(n(after.minute)-n(before.minute))));
  return Math.round((n(before.team1Gold)-n(before.team2Gold))+(n(after.team1Gold)-n(after.team2Gold)-(n(before.team1Gold)-n(before.team2Gold)))*ratio);
}

function enrichTimelineEvents(events,gold,duration){
  return events.map(event=>{const difference=timelineGoldAt(gold,n(event.minute)),afterDifference=timelineGoldAt(gold,Math.min(n(duration),n(event.minute)+2)),swing=afterDifference===null||difference===null?null:afterDifference-difference;return {...event,goldDifference:difference,goldLeader:difference===null?"UNKNOWN":publicTeamKey(meaningfulLead(difference,0)),leadGold:difference===null?null:Math.abs(difference),twoMinuteSwing:swing===null?null:Math.abs(swing),swingToward:swing===null?"UNKNOWN":publicTeamKey(meaningfulLead(swing,0))}});
}
...
function serverPhaseRows(set,publicEvents=[]){
  ...
  return ranges.filter(([from])=>from<duration).map(([from,to,label])=>{
    ...
    const phaseEvents=publicEvents.filter(event=>n(event.minute)>=from&&n(event.minute)<end).sort((a,b)=>(n(b.twoMinuteSwing)-n(a.twoMinuteSwing))||n(a.minute)-n(b.minute)).slice(0,3);
    return {label,fromMinute:from,toMinute:Number(end.toFixed(1)),startLeader:publicTeamKey(meaningfulLead(startDifference)),endLeader:publicTeamKey(meaningfulLead(endDifference)),endLeadGold:Math.abs(endDifference),swingToward:publicTeamKey(meaningfulLead(endDifference-startDifference,0)),swingGold:Math.abs(endDifference-startDifference),eventCount:events.filter(event=>n(event.minute)>=from&&n(event.minute)<end).length,keyEvents:phaseEvents};
  });
}
...
export function buildSeriesCommentaryPresentation(evidence){
  ...
  const teamNames={TEAM1:evidence.series.blueTeam||"1팀",TEAM2:evidence.series.redTeam||"2팀"},sets=(evidence.sets||[]).map(set=>{
    const teamTotals=set.teams||{},players=(set.players||[]).map(player=>({...player,team:publicTeamKey(player.side),impactScore:serverPlayerImpact(player,teamTotals[player.side]||{},player.side===set.winner)})).sort((a,b)=>b.impactScore-a.impactScore||a.name.localeCompare(b.name,"ko")),best=players[0]||null,worst=players.at(-1)||null,timeline=set.timeline||{},gold=(timeline.teamGold||[]).map(frame=>({minute:frame.minute,team1Gold:n(frame.BLUE),team2Gold:n(frame.RED),leader:publicTeamKey(meaningfulLead(n(frame.difference),0)),leadGold:Math.abs(n(frame.difference))})),soloKills=(timeline.killClusters||[]).filter(cluster=>cluster.sequence?.length===1).map(cluster=>({minute:cluster.sequence[0].minute,team:publicTeamKey(cluster.result),kind:"kill",type:"CHAMPION_KILL",label:`${cluster.sequence[0].killer} → ${cluster.sequence[0].victim}`})),rawEvents=[...soloKills,...(timeline.turningPoints||[]).map(point=>({minute:point.startMinute,team:publicTeamKey(point.result),kind:"fight",type:"TEAM_FIGHT",label:`교전 ${n(point.kills?.BLUE)}:${n(point.kills?.RED)}`})),...(timeline.objectives||[]).map(event=>publicTimelineEvent({...event,kind:"objective"})),...(timeline.buildings||[]).map(event=>publicTimelineEvent({...event,kind:"building"}))].filter(event=>event.minute!==null).sort((a,b)=>a.minute-b.minute),events=enrichTimelineEvents(rawEvents,gold,set.durationMinutes);
    return {setNumber:set.setNumber,gameId:set.gameId,winner:publicTeamKey(set.winner),durationMinutes:set.durationMinutes,dataAvailable:set.dataAvailable,mappingComplete:set.mappingComplete,sideMapping:{TEAM1:set.sideMapping?.BLUE||"UNKNOWN",TEAM2:set.sideMapping?.RED||"UNKNOWN"},teams:{TEAM1:{name:teamNames.TEAM1,...(teamTotals.BLUE||{})},TEAM2:{name:teamNames.TEAM2,...(teamTotals.RED||{})}},timeline:{available:Boolean(timeline.available),confidence:timeline.confidence?.overall||"unavailable",gold,events,phases:serverPhaseRows(set,events)},best:best?{id:best.id,name:best.name,team:best.team,role:best.roleLabel,champion:best.champion,impactScore:best.impactScore,kills:best.kills,deaths:best.deaths,assists:best.assists,killParticipation:best.killParticipation,damage:best.damage,gold:best.gold,cs:best.cs,vision:best.vision}:null,worst:worst?{id:worst.id,name:worst.name,team:worst.team,role:worst.roleLabel,champion:worst.champion,impactScore:worst.impactScore,kills:worst.kills,deaths:worst.deaths,assists:worst.assists,killParticipation:worst.killParticipation,damage:worst.damage,gold:worst.gold,cs:worst.cs,vision:worst.vision}:null};
  });
...
function publicRatingEvent(event,playerById){
  ...
  return {
    ...
    duoContextApplied:Boolean(event.duoContextApplied),duoChange:maybeNumber(event.duoChange),partnerAdjustment:maybeNumber(event.partnerAdjustment),timelineContextApplied:Boolean(event.timelineContextApplied),timelineChange:maybeNumber(event.timelineChange),performanceChange:maybeNumber(event.performanceChange),seriesGuardrailAdjustment:maybeNumber(event.seriesGuardrailAdjustment),defensive:Boolean(event.defensive),
  };
}
...
export function buildSeriesCommentaryEvidence({series,players=[],matches=[],seriesState=null,ratingVersion="",prepareMatches=null,playerResolver=null,positionScore=null}={}){
  ...
  const setReviews=importedSets.map((set,index)=>{
    ...
    const timeline=buildMatchTimelineEvidence({match,mapped,teamIdToSide}),laneRaw=evaluateBottomTimeline(match),laneTimeline=laneRaw?{version:laneRaw.version,confidence:laneRaw.confidence,teams:Object.fromEntries(Object.values(laneRaw.teams||{}).map(team=>[teamIdToSide.get(n(team.teamId))||String(team.teamId),{gold10:team.gold10,gold15:team.gold15,cs10:team.cs10,cs15:team.cs15,xp10:team.xp10,xp15:team.xp15,pureKills:team.pureKills,jungleKills:team.jungleKills,botDeathsToJungle:team.botDeathsToJungle,recovery:team.recovery,signal:team.signal}]))}:null,blueDuo=duo("BLUE"),redDuo=duo("RED"),higherLeader=key=>blueDuo[key]>redDuo[key]?"BLUE":redDuo[key]>blueDuo[key]?"RED":"EVEN",metricLeaders=Object.fromEntries(["kills","assists","damage","gold","cs","vision"].map(key=>[key,higherLeader(key)])),fewerDeaths=blueDuo.deaths<redDuo.deaths?"BLUE":redDuo.deaths<blueDuo.deaths?"RED":"EVEN",dimensionLeaders=[...new Set(Object.values(metricLeaders).filter(side=>side!=="EVEN"))],bottomDuoComparison={metricLeaders,fewerDeaths,mixed:dimensionLeaders.length>1};
    return {setNumber:n(set.number)||index+1,gameId:String(set.gameId),winner:set.winner,durationMinutes:Number((n(match.duration)/60).toFixed(1)),dataAvailable:true,mappingComplete:mapped.length===memberById.size,mappedPlayers:mapped.length,expectedPlayers:memberById.size,sideMapping,teams:{BLUE:teamTotals("BLUE"),RED:teamTotals("RED")},players:publicPlayers,matchups,bottomDuo:{BLUE:blueDuo,RED:redDuo,comparison:bottomDuoComparison,laneTimeline},objectives,timeline};
  });
