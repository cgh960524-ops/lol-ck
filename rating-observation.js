...
export function observe(p,opp,m,role,ref){
 ...
 let total=0,sum=0,pairedTotal=0,evidence=0;
 for(const [key,w] of Object.entries(WEIGHTS[role])){
  ...
  const absolute=clamp(az,-2.5,2.5),paired=clamp(az-bz,-3,3),signal=absolute;
  sum+=w*signal;pairedTotal+=w*paired;total+=w;evidence+=w*(ca?.n||0);
  metrics.push({key,label:LABELS[key],signal:Number(signal.toFixed(3)),absolute:Number(absolute.toFixed(3)),paired:Number(paired.toFixed(3)),weight:w});
 }
 const signal=total?sum/total:0;
 return {signal,pairedSignal:total?pairedTotal/total:0,quality:total,metrics,championEvidence:total?evidence/total:0,target:total?clamp(ref.center+ref.spread*signal,650,2850):null};
}
