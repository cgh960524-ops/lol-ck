...
function Get-InternalMatch([string]$gameId) {
    ...
    $soloKills=@{};$soloDeaths=@{};$timelineFile=Join-Path ([IO.Path]::GetTempPath()) ("eungck-timeline-"+[guid]::NewGuid().ToString("N")+".json")
    try { & curl.exe -s -k -u "riot:$($auth.Token)" --output $timelineFile "https://127.0.0.1:$($auth.Port)/lol-match-history/v1/game-timelines/$gameId"; $timeline=Get-Content -Raw -LiteralPath $timelineFile -Encoding UTF8|ConvertFrom-Json; foreach($frame in @($timeline.frames)){foreach($event in @($frame.events)){if($event.type -eq "CHAMPION_KILL" -and @($event.assistingParticipantIds).Count -eq 0){$killer=[string]$event.killerId;$victim=[string]$event.victimId;if([int]$event.killerId -gt 0){$soloKills[$killer]=1+[int]$soloKills[$killer]};if([int]$event.victimId -gt 0){$soloDeaths[$victim]=1+[int]$soloDeaths[$victim]}}}} } catch {} finally {if(Test-Path -LiteralPath $timelineFile){Remove-Item -LiteralPath $timelineFile -Force -ErrorAction SilentlyContinue}}
