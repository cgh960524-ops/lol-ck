...
function Get-InternalMatch([string]$gameId) {
    ...
    $soloKills=@{};$soloDeaths=@{};$epicObjectives=@();$timelineCollected=$false;$timelineFile=Join-Path ([IO.Path]::GetTempPath()) ("eungck-timeline-"+[guid]::NewGuid().ToString("N")+".json")
    try { & curl.exe -s -k -u "riot:$($auth.Token)" --output $timelineFile "https://127.0.0.1:$($auth.Port)/lol-match-history/v1/game-timelines/$gameId"; $timeline=Get-Content -Raw -LiteralPath $timelineFile -Encoding UTF8|ConvertFrom-Json;$timelineCollected=$true;$identityById=@{};$participantById=@{};foreach($identity in @($game.participantIdentities)){$identityById[[string]$identity.participantId]=$identity.player};foreach($participant in @($game.participants)){$participantById[[string]$participant.participantId]=$participant};foreach($frame in @($timeline.frames)){foreach($event in @($frame.events)){if($event.type -eq "CHAMPION_KILL" -and @($event.assistingParticipantIds).Count -eq 0){$killer=[string]$event.killerId;$victim=[string]$event.victimId;if([int]$event.killerId -gt 0){$soloKills[$killer]=1+[int]$soloKills[$killer]};if([int]$event.victimId -gt 0){$soloDeaths[$victim]=1+[int]$soloDeaths[$victim]}};if($event.type -eq "ELITE_MONSTER_KILL"){$killerId=[string]$event.killerId;$killerPlayer=$identityById[$killerId];$killerParticipant=$participantById[$killerId];$assistPuuids=@(@($event.assistingParticipantIds)|ForEach-Object{[string]$identityById[[string]$_].puuid}|Where-Object{$_});$epicObjectives+=@{timestamp=[long]$event.timestamp;monsterType=[string]$event.monsterType;monsterSubType=[string]$event.monsterSubType;teamId=[int]$(if($event.killerTeamId){$event.killerTeamId}elseif($event.teamId){$event.teamId}elseif($killerParticipant){$killerParticipant.teamId}else{0});killerPuuid=[string]$killerPlayer.puuid;assistingPuuids=$assistPuuids}}}} } catch {} finally {if(Test-Path -LiteralPath $timelineFile){Remove-Item -LiteralPath $timelineFile -Force -ErrorAction SilentlyContinue}}
