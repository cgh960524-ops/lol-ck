...
test("uploader timeline is bounded, persisted, preserved when omitted, and replaced by a later timeline",async()=>{
  ...
  try{
    ...
    const previousConsoleError=console.error;let adminRejected;try{console.error=()=>{};adminRejected=await request(handleRequest,"/api/ratings/recalculate",{method:"POST",headers:scopedHeaders,body:{}})}finally{console.error=previousConsoleError}assert.equal(adminRejected.status,401);
    ...
    let rejected;try{console.error=()=>{};rejected=await request(handleRequest,"/api/internal-matches/upload",{method:"POST",headers,body:{...match(gameId),timeline:{frameInterval:60_000,frames:[],events:[]}}})}finally{console.error=previousConsoleError}assert.equal(rejected.status,400);assert.match(rejected.body.error,/프레임/);
