...
(function (root) {
  'use strict';
  const VERSION = 'role-skill-v2.3.20260917';
  ...
  function recalculate(players,matches,seriesState={}) {
    ...
    function ensureRole(p,role) {
      ...
      if(!model.roles[role]){const prior=model.profile.roles[role],retention=soloRetention(model.games),hasInternal=Object.values(model.roles).some(r=>r.comparisons>0),transfer=hasInternal?model.overall:prior.rating,seed=clamp(prior.rating*retention+transfer*(1-retention),400,3200);model.roles[role]={rating:seed,seed,seedSource:hasInternal&&retention<1?'internal-transfer':'registration',registrationShare:hasInternal?retention:1,games:0,comparisons:0,weight:0,wins:0,performanceTotal:0,expectedTotal:0,opponents:new Set(),series:new Map(),recent:[],lastAt:0,confidence:0,uncertainty:330,prior:!!prior.peak};}
    ...
    for(const m of prepared){
      ...
      for(const e of entries){
        ...
        if(r){
          ...
          expectedPerformance=oppPre?matchupExpectation(before-oppPre.rating):0;
          ...
          priorChange=opponent&&obs.quality>0?(r.seed-before)*(learning/MATCHUP_SCALE)*priorStrength*12/(12+r.weight)*soloRetention(model.games,r.games)*obs.quality*independence:0;
      ...
      for(const step of pending){
        ...
        const afterOverall=overall(model);model.history.push({gameId:String(m.gameId),seriesId:m.seriesId,time:num(m.gameCreation),version:VERSION,role,comparisonStatus,internalGamesBefore,soloRetention:soloRetentionBefore,win:!!mp.win,before:Math.round(model.overall),after:Math.round(afterOverall),change:Math.round(afterOverall)-Math.round(model.overall),roleBefore:Math.round(before),roleAfter:r?Math.round(r.rating):null,roleChange:r?Math.round(r.rating)-Math.round(before):0,expected:Number(expected.toFixed(3)),performance:Number(obs.signal.toFixed(3)),expectedPerformance:opponent?Number(expectedPerformance.toFixed(3)):null,residual:Number(residual.toFixed(3)),opponentId:opponent?.p.id||null,opponentPower:opponent?Math.round(pre.get(opponent.id).rating):null,personalChange:Number(personal.toFixed(1)),outcomeChange:Number(outcome.toFixed(1)),priorChange:Number(priorChange.toFixed(1)),acceleration,quality:Number(obs.quality.toFixed(2)),reason:!role?'unconfirmed':!opponent?'no-opponent':residual>.12?'above':residual<-.12?'below':'expected',metrics:obs.metrics});model.overall=afterOverall;
      }
      diagnostics.matches++;updateBaselines(m,baseline);
    }
    for(const p of players){const model=models.get(String(p.id)),roles={};for(const role of ROLES)ensureRole(p,role);for(const [role,r] of Object.entries(model.roles)){roles[role]={rating:Math.round(r.rating),provisional:isProvisional(r),seed:Math.round(r.seed),seedSource:r.seedSource,soloRetention:soloRetention(model.games,r.games),comparisons:r.comparisons,games:r.games,wins:r.wins,confidence:Number(r.confidence.toFixed(3)),uncertainty:r.uncertainty,opponents:r.opponents.size,series:r.series.size,recentResidual:Number(mean(r.recent.slice(-5)).toFixed(3)),expectedTotal:r.expectedTotal,performanceTotal:r.performanceTotal};if(r.games)p.internalRoles[role]={...roles[role]};}p.ratingV2={version:VERSION,overall:Math.round(model.overall),roles,unconfirmed:model.unconfirmed,seed:model.seed,seedSource:model.profile.source,soloSeed:model.profile.solo,overallMethod:'internal-first',soloRetention:soloRetention(model.games),soloRestorationEnded:soloRetention(model.games)===0,comparisonCount:Object.values(model.roles).reduce((s,r)=>s+r.comparisons,0),provisional:Object.values(roles).filter(r=>r.games).length===0||Object.values(roles).filter(r=>r.games).reduce((s,r)=>s+r.games*r.confidence,0)/Math.max(1,p.internalGames)<.35||new Set(Object.values(model.roles).flatMap(r=>[...r.opponents])).size<3||p.internalGames<8};p.internalRating=Math.round(model.overall);p.ratingUncertainty=Math.round(mean(Object.values(roles).map(r=>r.uncertainty))||330);p.internalKda=p.internalGames?Number(((p.internalKills+p.internalAssists)/Math.max(1,p.internalDeaths)).toFixed(2)):0;p.ratingHistory=model.history.slice(-120);}
    return {players,matches:prepared,diagnostics};
  }
  root.CKRatingV23={VERSION,ROLES,soloRetention,matchupExpectation,initialProfile,isProvisional,confidenceLabel,basePower,overallScore,positionScore,confidence,confirmedRole,playerResolver,prepareMatches,recalculate};
})(globalThis);
