...
test('UNRANKED starts at a provisional 1350 in every unobserved role',()=>{const ps=players();ps[0].tier='UNRANKED';recalc(ps,[]);assert.equal(ps[0].ratingSeedV22.solo,1450);assert.equal(E.overallScore(ps[0]),1350);for(const role of E.ROLES){assert.equal(E.positionScore(ps[0],role),1350);assert.equal(ps[0].ratingV2.roles[role].games,0);assert.equal(ps[0].ratingV2.roles[role].provisional,true)}});
test('legacy unranked scores stay frozen unless that player explicitly opts in',()=>{const ps=players();ps[0].tier='UNRANKED';ps[0].ratingSeedV4={...structuredClone(E.initialProfile(ps[0])),policy:'matchup-v4',currentEvidence:{tier:'UNRANKED'},highTier:false,peakEvidence:null,roles:Object.fromEntries(E.ROLES.map(role=>[role,{rating:role==='TOP'?1450:1276,peak:null}]))};const saved=clone(ps[0].ratingSeedV4);recalc(ps,[]);assert.deepEqual(ps[0].ratingSeedV4,saved);assert.equal(E.positionScore(ps[0],'TOP'),1450);ps[0].unrankedPolicyOptIn='2026-09-21';recalc(ps,[]);assert.equal(E.positionScore(ps[0],'TOP'),1350);assert.deepEqual(ps[0].ratingSeedV4,saved)});
test('one existing unranked opt-in changes unplayed roles without rewriting prior sets',()=>{const ps=players();ps[0].tier='UNRANKED';const old=recalc(ps,[game(1),game(2)]),history=clone(old[0].ratingHistory),top=E.positionScore(old[0],'TOP'),other=old.map(p=>p.ratingV2.overall);old[0].unrankedPolicyOptIn='2026-09-21';recalc(old,[game(1),game(2)]);assert.deepEqual(old[0].ratingHistory,history);assert.equal(E.positionScore(old[0],'TOP'),top);assert.notEqual(E.positionScore(old[0],'MID'),1276);assert.deepEqual(old.map(p=>p.ratingV2.overall),other)});
test('unranked role evidence transfers cautiously and direct role play then learns independently',()=>{const ps=players();ps[0].tier='UNRANKED';const ms=Array.from({length:5},(_,i)=>{const m=game(i+1);m.gameCreation=Date.UTC(2026,8,22)+i*86400000;Object.assign(m.participants[0],{kills:10,deaths:1,assists:8,damage:26000,gold:14000,cs:220});return m});recalc(ps,ms);const top=ps[0].ratingV2.roles.TOP,mid=ps[0].ratingV2.roles.MID;assert.equal(top.games,5);assert.equal(mid.games,0);assert.ok(mid.rating>1350);assert.ok(mid.rating<top.rating);assert.equal(mid.seedSource,'unranked-transfer');assert.ok(ps[0].ratingHistory[0].acceleration>1);assert.ok(ps[0].ratingHistory.every(event=>Math.abs(event.roleChange)<=60));const m=game(6);m.gameCreation=Date.UTC(2026,8,27);[m.participants[0].role,m.participants[2].role]=['MID','TOP'];recalc(ps,[...ms,m]);assert.equal(ps[0].ratingV2.roles.MID.games,1);assert.equal(ps[0].ratingHistory.at(-1).roleBefore,mid.rating)});
...
test('same performance against stronger confirmed opponent gets more credit',()=>{const a=players(),b=players();b[5].tier='DIAMOND';const x=recalc(a,[game()])[0].ratingHistory[0],y=recalc(b,[game()])[0].ratingHistory[0];assert.equal(x.performance,y.performance);assert.ok(y.personalChange>x.personalChange);assert.ok(y.roleChange>x.roleChange);assert.ok(y.expectedPerformance<x.expectedPerformance);assert.ok(y.matchupChange>x.matchupChange);});
...
test('per-set ADC and support swap changes only the second set',()=>{const team=players(),series={history:[{id:'swap',blue:team.slice(0,5),red:team.slice(5),sets:[{number:1,gameId:'1',imported:true},{number:2,gameId:'2',imported:true,roleOverrides:{4:'SUPPORT',5:'ADC'}}]}]},rated=recalc(players(),[game(1),game(2)],series);assert.deepEqual(rated[3].ratingHistory.map(event=>event.role),['ADC','SUPPORT']);assert.deepEqual(rated[4].ratingHistory.map(event=>event.role),['SUPPORT','ADC']);assert.equal(rated[3].ratingV2.roles.ADC.games,1);assert.equal(rated[3].ratingV2.roles.SUPPORT.games,1)});
...
test('unplayed off-roles do not inherit main peak or changes from other roles',()=>{
 ...
 recalc(p,[game(),game(2)]);assert.equal(E.positionScore(p[0],'SUPPORT'),1810);assert.equal(p[0].ratingV2.roles.SUPPORT.evidenceRating,1810);assert.equal(p[0].ratingV2.roles.SUPPORT.games,0);assert.equal(p[0].ratingV2.roles.SUPPORT.provisional,true);
});
...
test('master off-role retains general skill; unknown evidence is provisional, not platinum',()=>{
 ...
 assert.equal(E.positionScore(p[0],'TOP'),2050);assert.equal(E.positionScore(p[0],'JUNGLE'),1810);assert.equal(E.positionScore(p[0],'SUPPORT'),1810);assert.equal(p[0].ratingV2.roles.SUPPORT.evidenceRating,1810);
...
test('first unfamiliar-role game does not mechanically deduct the role offset from overall',()=>{
 ...
 recalc(p,[m]);const change=p[0].ratingHistory[0].roleChange;assert.equal(p[0].ratingV2.overall,p[0].ratingSeedV4.roles.TOP.rating);assert.equal(change,0);assert.equal(p[0].ratingV2.roles.MID.rating,p[0].ratingV2.roles.MID.evidenceRating);
});
...
test('mature unplayed off-role starts conservatively without changing internal overall',()=>{
 ...
 assert.notEqual(p.ratingV2.overall,p.ratingSeedV22.solo);const mid=p.ratingV2.roles.MID;assert.equal(mid.evidenceRating,p.ratingV2.overall);assert.equal(mid.rating,mid.evidenceRating);assert.equal(mid.calibrationDiscount,0);assert.equal(mid.seedSource,'internal-transfer');assert.equal(mid.games,0);assert.equal(mid.comparisons,0);assert.equal(mid.confidence,0);assert.equal(mid.provisional,true);
