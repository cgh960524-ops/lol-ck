...
test("finishing a series automatically calls Responses API once and public GET reads the stored review",async()=>{
  ...
  globalThis.fetch=async(url,options={})=>{
    ...
    const review={headline:responseHeadline,overview:"저장된 경기 지표를 근거로 작성한 테스트 총평입니다.",decisiveFactors:["승리 팀의 피해량 우위"],setReviews:[{setNumber:1,title:"첫 세트",summary:"지표상 우위를 만들었습니다.",team1Good:"1팀의 장점을 설명합니다.",team2Good:"2팀의 장점을 설명합니다.",bestReason:"서버 선정 BEST 근거를 설명합니다.",worstReason:"서버 선정 WORST 근거를 설명합니다."}],matchupReviews:[{role:"원딜",title:"바텀 구도",summary:"맞포지션과 2대2 지표를 함께 봤습니다."}],notablePlayers:[{name:"테스트 선수",side:"RED",summary:"확인 가능한 지표에서 눈에 띄었습니다."}],ratingSummary:"확정된 롤력 변화를 설명합니다.",dataNotice:"기록되지 않은 오더와 교전 과정은 단정하지 않습니다."};
  ...
  try{
    ...
    const fetched=await request(handleRequest,`/api/series-commentary?seriesId=${seriesId}`);assert.equal(fetched.status,200);assert.equal(fetched.body.status,"ready");assert.equal(fetched.body.review.headline,"한타 집중력이 가른 시리즈");assert.ok(fetched.body.analysis?.sets?.length);assert.equal(calls,1);
    ...
    const reviewedHeadline="사람이 검수한 타임라인 총평",reviewed=await request(handleRequest,"/api/series-commentary/regenerate",{method:"POST",headers:{authorization:"Bearer test-upload-token"},body:{seriesId,review:{...forced.review,headline:reviewedHeadline}}});assert.equal(reviewed.status,200);assert.equal(reviewed.body.status,"ready");assert.equal(reviewed.body.review.headline,reviewedHeadline);assert.equal(calls,5);
