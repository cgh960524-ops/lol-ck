...
test("finishing a series automatically calls Responses API once and public GET reads the stored review",async()=>{
  ...
  globalThis.fetch=async(url,options={})=>{
    ...
    return new Response(JSON.stringify({model:"test-model",output:[{type:"message",content:[{type:"output_text",text:JSON.stringify(review)}]}],usage:{input_tokens:100,output_tokens:50,total_tokens:150}}),{status:200,headers:{"Content-Type":"application/json"}});
  };
  try{
    ...
    assert.equal(calls,1);assert.equal(requestBody.store,false);assert.equal(requestBody.reasoning.effort,"low");assert.equal(requestBody.text.format.type,"json_schema");assert.equal(requestBody.model,"test-model");
