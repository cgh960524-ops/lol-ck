...
let browser;
try{
 const expected=await readFile(new URL('../downloads/eungck-uploader-20260919.zip',import.meta.url));
 ...
 const downloading=page.waitForEvent('download');await page.locator('.uploader-download').click();const download=await downloading;assert.equal(download.suggestedFilename(),'eungck-uploader-20260919.zip');assert.deepEqual(await readFile(await download.path()),expected);assert.equal(await page.locator('.topbar nav .active').getAttribute('href'),'#players');assert.deepEqual(errors,[]);
